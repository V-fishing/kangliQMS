...
// SPC 判异规则与预警通知配置（共享组件）：
// 统一收口到「系统管理 / 配置中心」，SPC 模块的「判异规则」页复用本组件（薄壳），单一数据源。
import { ref, onMounted, computed } from 'vue'
import { ElMessage } from 'element-plus'
import { spcApi } from '@/api'
import type { SpcRuleCfg } from '@/types/spc'

const ALARM_CFG_KEY = 'qms.spc.alarmNotifyCfg'

const rules = ref<SpcRuleCfg[]>([])
onMounted(async () => {
  try {
    rules.value = await spcApi.getRulesCfg()
  } catch {
    rules.value = []
  }
  // 预警分级与通知：本地持久化（后端暂无对应接口）
  try {
    const saved = localStorage.getItem(ALARM_CFG_KEY)
    if (saved) {
      const v = JSON.parse(saved)
      if (v.alarmLevel) alarmLevel.value = v.alarmLevel
      if (typeof v.suppressMin === 'number') suppressMin.value = v.suppressMin
      if (v.notify) notify.value = { ...notify.value, ...v.notify }
    }
  } catch {
    // 忽略本地缓存解析失败
  }
...
// 单条规则启用/停用（至少保留一条）
async function toggle(r: SpcRuleCfg) {
  const next = !r.on
  if (!next && enabledCount.value <= 1) {
    ElMessage.warning('至少需保留一条判异规则，无法全部停用')
    return
  }
  if (!r.rid) { ElMessage.error('规则缺少后端标识，无法更新'); return }
  const prev = r.on
  r.on = next // 乐观更新
  try {
    await spcApi.toggleRule(r.rid, next)
    ElMessage.success(`${r.id} 已${next ? '启用' : '停用'}`)
  } catch (e: any) {
    r.on = prev // 失败回滚
    ElMessage.error(e?.msg || e?.message || '规则状态更新失败')
  }
}
...
function save() {
  localStorage.setItem(ALARM_CFG_KEY, JSON.stringify({
    alarmLevel: alarmLevel.value,
    suppressMin: suppressMin.value,
    notify: notify.value,
  }))
...
.spc-rule-config { display: flex; flex-direction: column; gap: 14px; }
