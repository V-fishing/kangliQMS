...
// SPC 参数配置（共享组件）：参数 CRUD + 基线控制限 + CPK 计算配置。
// 统一收口到「系统管理 / 配置中心」，原 SPC「参数配置」页已删除，单一入口。
...
const list = ref<(SpcParam & { id: string })[]>([])
onMounted(async () => {
  try {
    list.value = await spcApi.getParams()
  } catch {
    list.value = []
  }
})
...
function openEdit(r: SpcParam & { id: string }) { model.value = { ...r }; dialog.value = true }
function onSubmit(v: Record<string, unknown>) {
  const idx = list.value.findIndex((x) => x.p === v.p)
  if (idx >= 0) list.value[idx] = { ...list.value[idx], ...(v as SpcParam & { id: string }) }
  else list.value.unshift(v as SpcParam & { id: string })
  dialog.value = false
  ElMessage.success('参数已保存')
}
function onDelete(r: SpcParam & { id: string }) {
...
.spc-param-config { display: flex; flex-direction: column; gap: 14px; }
.check-list {
  display: flex;
